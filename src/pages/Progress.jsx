...
export default function Progress() {
  ...
  const { data: workouts } = useQuery({
    ...
    queryFn: async () => {
      const all = await api.entities.WorkoutCompletion.list(
        "-completed_date",
        100,
      );
      return all;
  ...
  const { data: pointsTransactions = [] } = useQuery({
    ...
    queryFn: () =>
      api.entities.PointsTransaction.list("-transaction_date", 500),
    staleTime: 1000 * 60 * 5,
    gcTime: 1000 * 60 * 30,
  });
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
            ...
            <div className="flex items-end justify-between">
              <p className="text-2xl font-bold text-blue-400">
                {streaks[0]?.current_streak || 0}w
              </p>
              {streakChange.value !== 0 && (
                <div
                  className={`flex items-center gap-1 text-xs font-semibold ${streakChange.value > 0 ? "text-green-400" : "text-red-400"}`}
                >
                  {streakChange.value > 0 ? (
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
            ...
            <div className="flex items-end justify-between">
              ...
              {pointsChange.percentage !== null && (
                <div
                  className={`flex items-center gap-1 text-xs font-semibold ${pointsChange.value > 0 ? "text-green-400" : pointsChange.value < 0 ? "text-red-400" : "text-zinc-500"}`}
                >
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {workoutChange.value !== 0 && (
                  <div
                    className={`flex items-center gap-1 text-sm ${workoutChange.value > 0 ? "text-green-400" : "text-red-400"}`}
                  >
                    {workoutChange.value > 0 ? (
                    ...
                    <span className="font-semibold">
                      {Math.abs(workoutChange.value)} this week
                    </span>
                    <span className="text-zinc-500">
                      vs {workoutChange.previousWeek} last week
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {calorieChange.percentage !== null && (
                  <div
                    className={`flex items-center gap-1 text-sm ${calorieChange.value > 0 ? "text-green-400" : calorieChange.value < 0 ? "text-red-400" : "text-zinc-500"}`}
                  >
                    {calorieChange.value > 0 ? (
                      <ArrowUp className="w-4 h-4" />
                    ) : calorieChange.value < 0 ? (
                      <ArrowDown className="w-4 h-4" />
                    ) : (
                    ...
                    <span className="font-semibold">
                      {Math.abs(calorieChange.percentage)}% vs last week
                    </span>
                    <span className="text-zinc-500">
                      ({Math.round(calorieChange.thisWeek)} avg)
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {weightChange.value !== 0 && (
                  <div
                    className={`flex items-center gap-1 text-sm ${weightChange.value > 0 ? "text-red-400" : "text-green-400"}`}
                  >
                    {weightChange.value > 0 ? (
                      <ArrowUp className="w-4 h-4" />
                    ) : (
                    ...
                    <span className="font-semibold">
                      {Math.abs(weightChange.value).toFixed(1)} kg{" "}
                      {weightChange.value > 0 ? "gained" : "lost"}
                    </span>
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
                    ...
                    <p className="text-2xl font-bold text-white">
                      {weightChange.current.toFixed(1)} kg
                    </p>
                  </div>
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
                    <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">
                    ...
                    <p className="text-2xl font-bold text-blue-400">
                      {weightChange.average.toFixed(1)} kg
                    </p>
                  </div>
                  <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
                    <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">
              ...
              <p className="text-2xl font-bold text-blue-400">
                {activeGoals.length > 0
                  ? Math.round(
                      activeGoals.reduce(
                        (sum, g) =>
                          sum + (g.current_value / g.target_value) * 100,
                        0,
                      ) / activeGoals.length,
                    )
                  : 0}
              ...
              <p className="text-2xl font-bold text-purple-400">
                {
                  activeGoals.filter((g) => {
                    ...
                    const daysTotal = Math.ceil(
                      (new Date(g.target_date) - new Date(g.start_date)) /
                        (1000 * 60 * 60 * 24),
                    );
          ...
          {activeGoals.length === 0 && !showGoalForm ? (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-12 text-center">
              ...
              <p className="text-zinc-500 mb-4">
                No active goals yet. Set one to get started!
              </p>
              <Button
                onClick={() => setShowGoalForm(true)}
...
function generatePointsChartData(transactions) {
  ...
  const meaningful = transactions.filter(
    (t) => t.points_awarded > 0 && t.source !== "daily_calc",
  );
...
function generateLeaderboardChartData(points) {
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    const estimatedRank = Math.max(
      1,
      50 - (points.total_points || 0) / 100 + Math.random() * 20,
    );

    weeks.push({
      week: date.toLocaleDateString("en-GB", {
        month: "short",
        day: "numeric",
      }),
...
function generateMacroData(mealLogs) {
  ...
  const recentLogs = mealLogs.filter(
...
function calculateCalorieChange(mealLogs) {
  ...
  const thisWeekAvg =
    thisWeekLogs.length > 0
      ? thisWeekLogs.reduce((sum, log) => sum + (log.calories || 0), 0) /
        Math.max(thisWeekLogs.length / 3, 1)
      : 0;

  const lastWeekAvg =
    lastWeekLogs.length > 0
      ? lastWeekLogs.reduce((sum, log) => sum + (log.calories || 0), 0) /
        Math.max(lastWeekLogs.length / 3, 1)
      : 0;

  if (lastWeekAvg === 0)
    return { value: 0, percentage: null, thisWeek: thisWeekAvg };
...
function calculateWeightChange(weightLogs) {
  ...
  const average =
    weightLogs.reduce((sum, log) => sum + (log.weight_kg || 0), 0) /
    weightLogs.length;
