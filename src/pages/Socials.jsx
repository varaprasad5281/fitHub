...
export default function SocialsPage() {
  ...
  const hasEliteAccess = checkElite(
    activeSub(Array.isArray(subscription) ? subscription : []),
  );
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-zinc-500">
              Connect with serious athletes - upgrade Elite to build your
              network
            </p>
          </div>
          <SocialsPreview />
  ...
  const leaderboard = [myEntry, ...friends].sort(
    (a, b) => (b.weekly_points || 0) - (a.weekly_points || 0),
  );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-8">
          <TabsList className="w-full bg-zinc-900/50 border border-zinc-800 mb-6 grid grid-cols-4 rounded-xl overflow-hidden p-0">
            <TabsTrigger
              value="friends"
              className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 rounded-none h-full py-2.5 data-[state=active]:bg-amber-500/10 data-[state=active]:text-amber-400 data-[state=active]:shadow-none"
            >
                ...
                <div className="space-y-2">
                  {outgoing.map((request, idx) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-zinc-500 bg-zinc-800 px-2 py-1 rounded-full ml-2">
                        Pending
                      </span>
                    </motion.div>
                  ))}
                  ...
                  {leaderboard.map((entry, idx) => (
                    ...
                    >
                      ...
                      {entry.avatar_url ? (
                        ...
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0 ${entry.isMe ? "bg-amber-600" : "bg-zinc-700"}`}
                        >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {entry.isMe && (
                            <span className="text-[10px] font-bold bg-zinc-600 text-zinc-300 px-1.5 py-0.5 rounded-full flex-shrink-0">
                              YOU
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <BadgeMiniRow
                            badges={socialBadgeMap[entry.email] || []}
                            size="xs"
                            align="left"
                            above={true}
                          />
          ...
          <TabsContent value="chat">
            {selectedChat ? (
              <ChatWindow
                ...
                friendName={
                  conversations.find((c) => c.friend_email === selectedChat)
                    ?.friend_username ||
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
                ...
                <div className="space-y-2">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <p
                                className={`text-sm truncate ${hasUnread ? "font-bold text-white" : "font-semibold text-white"}`}
                              >
                                {conv.friend_username || "Unknown"}
                              </p>
                              {timeStr && (
                                <span
                                  className={`text-[10px] flex-shrink-0 ${hasUnread ? "text-amber-400 font-semibold" : "text-zinc-600"}`}
                                >
                                  {timeStr}
                                </span>
                              )}
                            ...
                            <p
                              className={`text-xs truncate ${hasUnread ? "text-zinc-300" : "text-zinc-500"}`}
                            >
                  ...
                  {friends
                    .filter(
                      (f) =>
                        !conversations.some((c) => c.friend_email === f.email),
                    )
                    .map((friend) => (
                      <button
