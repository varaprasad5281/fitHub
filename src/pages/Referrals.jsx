...
function MilestoneCard({ milestone, completed, granted }) {
  ...
  const progress = Math.min(
    100,
    Math.round((completed / milestone.count) * 100),
  );

  return (
    <div
      className={`rounded-2xl border p-5 transition-all ${achieved ? `${s.border} ${s.bg} shadow-lg` : "border-zinc-800 bg-zinc-900/50"}`}
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          <span
            className={`text-3xl ${!achieved ? "grayscale opacity-40" : ""}`}
          >
            {milestone.icon}
          </span>
          <div>
            <p
              className={`font-bold text-sm ${achieved ? "text-white" : "text-zinc-400"}`}
            >
              {milestone.label}
            </p>
            <p
              className={`text-xs font-semibold mt-0.5 ${achieved ? s.label : "text-zinc-600"}`}
            >
              {milestone.reward}
            </p>
          </div>
        ...
        {granted ? (
          ...
          <span
            className={`text-xs font-semibold px-2 py-1 rounded-full ${s.badge}`}
          >
            Unlocked
          </span>
        ) : null}
      ...
      <p
        className={`text-xs mb-3 ${achieved ? "text-zinc-300" : "text-zinc-600"}`}
      >
        {milestone.detail}
      </p>
