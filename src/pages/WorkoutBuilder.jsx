...
const calculateWorkoutPoints = (workout) => {
  return (
    (pointsPerDifficulty[workout.difficulty] || 10) *
...
export default function WorkoutBuilder() {
  ...
  const { data: workouts, isLoading: workoutsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const filtered = all.filter(
        (w) => !w.week_number || w.week_number === currentWeek,
      );
  ...
  const generateMutation = useMutation({
    mutationFn: async ({ params, type }) => {
      return withActionDebug(
        "Generate " + (type === "weekly" ? "Weekly" : "Single") + " Workout",
        async () => {
          if (!user) {
          ...
          if (workouts && workouts.length > 0) {
            await Promise.all(
              workouts.map(function (w) {
                return api.entities.Workout.delete(w.id);
              }),
            );
          }
          ...
          if (type === "weekly") {
            return api.functions.invoke(
              "generateWeeklyWorkout",
              Object.assign({}, params, goalData),
            );
          } else {
            return api.functions.invoke(
    ...
    mutationFn: async (workoutId) => {
      return withActionDebug(
        ...
        },
        {
          onError: () =>
            toast.error(
              "Could not record workout completion. Please try again.",
            ),
        },
      )();
    },
  ...
  const isWorkoutCompletedToday = (workoutId) => {
    ...
    return completions.some(
      (c) => c.workout_id === workoutId && c.completed_date === today,
    );
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500">
            AI-generated personalized training routines based on your goals
          </p>
            ...
            <div className="flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 p-4 rounded-xl bg-zinc-900/50 border border-zinc-800">
                  {planType === "single" && (
                    <div>
                      ...
                      <Select
                        value={workoutParams.focus}
                        onValueChange={(v) =>
                          setWorkoutParams({ ...workoutParams, focus: v })
                        }
                      >
                        <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white">
                          <SelectValue
                            placeholder={`Default: ${profile?.workout_preference?.replace(/_/g, " ") || "Mixed"}`}
                          />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {planType === "weekly" && (
                    <div>
                      ...
                      <Select
                        value={workoutParams.days_per_week}
                        onValueChange={(v) =>
                          setWorkoutParams({
                            ...workoutParams,
                            days_per_week: v,
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setWorkoutParams({ ...workoutParams, duration: v })
                      }
                    >
                      <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white">
                        <SelectValue
                          placeholder={
                            `${profile?.workout_duration || 30} min` ||
                            "Select duration"
                          }
                        />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setWorkoutParams({ ...workoutParams, difficulty: v })
                      }
                    >
                      <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white">
                        <SelectValue
                          placeholder={
                            profile?.experience_level || "Select difficulty"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    ...
                    <Select
                      value={workoutParams.target_muscles}
                      onValueChange={(v) =>
                        setWorkoutParams({
                          ...workoutParams,
                          target_muscles: v,
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setWorkoutParams({ ...workoutParams, equipment: v })
                      }
                    >
                      <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white">
                        <SelectValue
                          placeholder={
                            profile?.available_equipment || "Select equipment"
                          }
                        />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  ...
                  onClick={() => {
                    ...
                    const params = Object.fromEntries(
                      Object.entries(workoutParams).filter(([k, v]) => {
                        if (planType === "single" && k === "days_per_week")
                          return false;
                        if (planType === "weekly" && k === "focus")
                          return false;
                        return v !== "";
                      }),
                    );
                    ...
                    generateMutation.mutate({
                      params:
                        Object.keys(params).length > 0
                          ? enhancedParams
                          : {
                              user_preferences: enhancedParams.user_preferences,
                            },
                      type: planType,
                    });
                  }}
                ...
                >
                  ...
                  {workouts.length > 0
                    ? `Regenerate ${planType === "weekly" ? "Weekly Plan" : "Workout"}`
                    : `Generate ${planType === "weekly" ? "Weekly Plan" : "Workout"}`}
                </Button>
                <Button
          ...
          cachedWorkouts.length === 0 ? (
            ...
            (() => {
              const weeklyWorkouts = (
                workouts.length > 0 ? workouts : cachedWorkouts
              ).filter((w) => w.day_of_week);
              if (!weeklyWorkouts.length)
                return (
                  <div className="text-center py-12 rounded-xl border border-zinc-800 bg-zinc-900/30">
                    ...
                    <p className="text-zinc-500">
                      No weekly plan yet. Generate your weekly plan above!
                    </p>
                  </div>
                );
              return (
                          ...
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2 text-xs">
                                ...
                                <span className="px-2 py-1 rounded-full bg-yellow-500/20 text-yellow-400 flex items-center gap-1">
                                  ...
                                  {calculateWorkoutPoints(workout)} pts
                                </span>
                              </div>
                            </div>
                            {isWorkoutCompletedToday(workout.id) ? (
                              ...
                              >
                                ...
                                {getWorkoutCompletionCount(workout.id) > 0
                                  ? "Complete Again"
                                  : "Complete"}
                              </Button>
                            )}
            ...
            (() => {
              ...
              if (!singleWorkouts.length)
                return (
                  <div className="text-center py-12 rounded-xl border border-zinc-800 bg-zinc-900/30">
                    <Dumbbell className="w-12 h-12 text-zinc-600 mx-auto mb-3" />
                    <p className="text-zinc-500">
                      No workout yet. Generate your personalized workout above!
                    </p>
                  </div>
                );
              return (
