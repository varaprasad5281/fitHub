...
export default function Leaderboard() {
  ...
  const { data: leaderboardData, isLoading: leaderboardLoading } = useQuery({
    ...
    queryFn: () =>
      api.functions.invoke("getLeaderboardCachedSecure", {
        timeframe,
        category,
      }),
    staleTime: 1000 * 30,
  ...
  const { data: friends = [], isLoading: friendsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.functions.invoke("getFriendsList", {
        type: "accepted",
      });
      return res?.friends || [];
    },
  ...
  if (
    subLoading ||
    (leaderboardLoading && !leaderboardData?.leaderboard?.length)
  ) {
    return (
  ...
  if (!hasAccess) {
    ...
    if (isPending) {
      let message =
        "Your payment is being processed. You'll have access to the leaderboard in a few minutes. Check back soon! 🔥";
      return (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-zinc-500">
              See where you stand among the 7% - upgrade to Elite to claim your
              position
            </p>
          </div>
          <LeaderboardPreview
  ...
  const displayRest = pinnedUser
    ? rest.filter((item) => item.created_by !== user?.email)
    : rest;
      ...
      <div className="max-w-4xl mx-auto">
                    ...
                    <div className="rounded-xl border-2 border-amber-500/50 bg-amber-500/10 p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <p className="text-white font-semibold">
                          {pinnedUser.username || "You"}{" "}
                          <span className="text-amber-400 text-xs">(You)</span>
                        </p>
                        <p className="text-zinc-500 text-xs">
                          {dynamicMessage?.text ||
                            `Level ${pinnedUser.current_level}`}
                        </p>
                      </div>
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xl font-bold text-amber-400">
                          {category === "points"
                            ? timeframe === "weekly"
                              ? pinnedUser.weekly_points
                              : pinnedUser.total_points
                            : category === "workouts"
        ...
        <div className="mt-8 p-4 rounded-xl bg-zinc-900/30 border border-zinc-800/50">
          <p className="text-zinc-600 text-xs leading-relaxed">
            {timeframe === "weekly" ? (
              <>
                <strong className="text-zinc-500">Weekly Reset:</strong>{" "}
                Leaderboard resets every Monday. Top 3 earn exclusive badges.
                Stay disciplined. 🏆
              </>
            ) : (
              <>
                <strong className="text-zinc-500">All-Time:</strong> Total
                points accumulated since you joined. Legends are made here. 💎
              </>
            )}
          </p>
        </div>
