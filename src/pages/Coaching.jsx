...
import {
  Sparkles, Lightbulb, Loader2, History, X, Star,
...
export default function Coaching() {
  ...
  const { data: coaching = [], isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      const sessions = /** @type {any[]} */ (await api.entities.CoachingSession.list());
  ...
  // Derive sessions
  const candidateCoaching = coaching.find(c => c.session_type === 'daily_candidate' && c.session_date === today);
  ...
  const generateCoachingMutation = useMutation({
    ...
    onError: () => toast.error('Failed to generate coaching. Please try again.'),
  });
  ...
  const resolveReviewMutation = useMutation({
    ...
    onSuccess: (_, { action }) => {
      ...
      if (action === 'accept') toast.success('New coaching plan adopted! +10 pts');
      else if (action === 'reject') toast.success('Kept your current coaching plan');
  ...
  return (
    <div className="min-h-screen bg-zinc-950 p-4 sm:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {weeklyCoaching && (
          <div className="rounded-2xl border border-blue-500/30 bg-blue-500/5 p-6 sm:p-8 mb-8">
            ...
            <CoachingCard
              ...
              onFeedback={(feedback) => resolveReviewMutation.mutate({
                action: 'feedback',
        ...
        {/* Daily Coaching */}
        <div data-coaching-section className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-6 sm:p-8 mb-8">
          ...
          {showInfo && (
            <div className="mb-5 p-4 rounded-xl bg-zinc-800/70 border border-zinc-700 text-sm text-zinc-300 leading-relaxed">
              <p className="font-semibold text-white mb-1">How coaching plans work</p>
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border-2 border-green-500/40 bg-green-500/5 p-1">
                <div className="flex items-center gap-2 px-3 pt-3 pb-1">
                  <span className="text-[10px] font-bold text-green-400 uppercase tracking-widest bg-green-500/20 px-2 py-0.5 rounded-full">New Plan Ready</span>
                  <span className="text-xs text-zinc-500">Review this before we update your plan</span>
                </div>
                <div className="p-3">
                ...
                <div className="flex gap-3 px-3 pb-3">
                  <Button
                    onClick={() => resolveReviewMutation.mutate({ action: 'accept' })}
                  ...
                  <Button
                    onClick={() => resolveReviewMutation.mutate({ action: 'reject' })}
                    disabled={resolveReviewMutation.isPending}
                  ...
                  >
                    {resolveReviewMutation.isPending ? (
                      ...
                      <><ThumbsDown className="w-4 h-4 mr-2" /> Keep Current Plan</>
                    )}
                  </Button>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                >
                  {generateCoachingMutation.isPending ? (
                    ...
                    <><Sparkles className="w-4 h-4 mr-2" /> Refresh Coaching Plan</>
                  )}
                </Button>
              </div>
            </div>
