...
export default function Subscription() {
  ...
  useEffect(() => {
    ...
    if (params.get("checkout") === "success" && sessionId) {
      ...
      api.functions
        .invoke("verifyCheckoutSession", { session_id: sessionId })
        .then(() => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const active = list.find(
        (s) => s.stripe_subscription_id && (s.plan === "starter" || !s.plan),
      );
      if (active) {
        api.functions
  ...
  const isElite =
    subscription?.plan === "elite_monthly" ||
    subscription?.plan === "elite_yearly";
  ...
  const handleUpgrade = async (billingPeriod) => {
    if (window.self !== window.top) {
      toast.error(
        "Checkout only works from published apps. Please open this app in a new tab.",
      );
      return;
    }
    ...
    await withActionDebug(
      "Subscription Upgrade " + billingPeriod,
      async () => {
        ...
        const idempotencyKey =
          "checkout_" + user.email + "_" + billingPeriod + "_" + Date.now();
    ...
    await withActionDebug(
      ...
      async () => {
        ...
        if (url) {
          ...
          throw new Error(
            "Could not open billing management. Please try again or contact support.",
          );
        }
      },
      {
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 mb-4">
                ...
                <div>
                  ...
                  <p className="text-zinc-400 text-xs">
                    No charge yet. Your card will be billed automatically when
                    the trial ends. Cancel anytime before then.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 mb-4">
                ...
                <div>
                  ...
                  <ul className="text-zinc-400 text-xs space-y-1">
                    ...
                    {!isElite && (
                      <li>
                        • ⚠️ Social &amp; competitive features not included -
                        upgrade to Elite
                      </li>
                    )}
      ...
      {history?.subscriptionHistory &&
        history.subscriptionHistory.length > 0 && (
          <div className="mt-12">
