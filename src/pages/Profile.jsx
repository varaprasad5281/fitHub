    ...
    return (
      <div className="text-center py-4">
        <p className="text-xs text-zinc-500">
          No active goals. Set one on the Progress page!
        </p>
      </div>
    );
  }
...
export default function Profile() {
    ...
    if (user) {
      ...
      if (params.get("checkout") === "success") {
        ...
        api.analytics.track({
          ...
          properties: { source: "checkout_success" },
        });
  ...
  const { data: subscriptions = [] } = useQuery({
    ...
    queryFn: () =>
      api.entities.Subscription.filter({ created_by: user?.email }),
    enabled: !!user,
    staleTime: 1000 * 60 * 5,
  });
  ...
  const calculateDailyPoints = async () => {
    await withActionDebug(
      ...
      },
      {
        ...
        onError: () =>
          toast.error("Could not recalculate points. Please try again."),
      },
    )();
  };
          ...
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-8 sm:p-12">
            ...
            <div className="space-y-3">
              <Button
                ...
                className="w-full bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-black font-semibold h-12 rounded-full"
              >
    ...
    <div className="min-h-screen bg-zinc-950 p-4 sm:p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        >
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1">
                  ...
                  {hasPaidAccess && activeSubscription?.plan && (
                    <span
                      className={`px-2 py-0.5 rounded-full border text-xs font-semibold flex items-center gap-1 ${planColors[activeSubscription.plan] || planColors.starter_monthly}`}
                    >
                      <Crown className="w-3 h-3" />
                      {planLabels[activeSubscription.plan] ||
                  ...
                  <div className="mt-2 w-40">
                    ...
                    <div className="h-1.5 rounded-full bg-zinc-800 overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, pointsData.progressPercentage || 0)}%`,
                        }}
                      />
                    </div>
                    <p className="text-xs text-zinc-600 mt-1">
                      {pointsData.pointsInLevel || 0} /{" "}
                      {pointsData.pointsNeededForNextLevel || 100} XP
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap">
              ...
              <Link to={createPageUrl("Subscription")}>
                <Button
                  variant="outline"
                  size="sm"
                  className="border-amber-500/40 text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 hover:border-amber-400 hover:text-amber-400 rounded-full"
                >
                  {t("profile.manage")}
                </Button>
              </Link>
          ...
          {profile?.bio && (
            <p className="text-zinc-400 text-sm mt-4 pt-4 border-t border-zinc-800">
              {profile.bio}
            </p>
          )}
        </motion.div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <PointsProgressCard
              ...
              pointsNeededForNextLevel={
                pointsData.pointsNeededForNextLevel || 100
              }
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-1">
                {days.map((day, i) => {
                  ...
                  const hasWorkout = workoutCompletions.some(
                    (w) =>
                      new Date(w.completed_date).toDateString() ===
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    {earnedBadges.length > 0 && (
                      <span className="text-xs bg-amber-500/20 text-amber-400 font-semibold px-1.5 py-0.5 rounded-full">
                        {earnedBadges.length}
                      </span>
                    )}
                  </div>
                  <Link to={createPageUrl("Badges")}>
                    <span className="text-xs text-amber-400 hover:text-amber-300 font-semibold">
                      {t("profile.viewAll")}
                    </span>
                  </Link>
                </div>
                ...
                {earnedBadges.length === 0 ? (
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {earnedBadges.slice(0, 8).map((badge) => {
                      const r =
                        BADGE_RARITY[badge.rarity_level] || BADGE_RARITY.common;
                      return (
                        <div
                        ...
                        >
                          <div
                            className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl border ${r.bg} ${r.border} ${badge.rarity_level === "legendary" ? "animate-pulse-glow" : ""} transition-transform group-hover:scale-110`}
                          >
                            {badge.icon || "🏅"}
                          </div>
                          <span
                            className={`text-[10px] font-medium text-center leading-tight ${r.label} line-clamp-1 w-full px-0.5`}
                          >
                            {badge.name}
                          </span>
                        </div>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Link to={createPageUrl("Progress")}>
                    <span className="text-xs text-purple-400 hover:text-purple-300 font-semibold">
                      View All
                    </span>
                  </Link>
                </div>
