...
function EarnedCard({ badge, onFeature, featured }) {
  ...
  return (
    <div
      className={`group relative rounded-2xl border ${r.border} ${r.bg} shadow-lg ${r.glow} p-5 flex flex-col items-center gap-2 text-center h-[194px]`}
    >
      {featured && (
      ...
      <p className={`text-xs font-semibold uppercase tracking-wide ${r.label}`}>
        {badge.rarity_level}
      </p>
      ...
      <div className="relative w-full">
        ...
        >
          ...
          <div
            className={`rounded-xl border ${r.border} bg-zinc-950 shadow-2xl p-3 text-left`}
          >
          ...
          <div className="flex justify-center">
            <div
              className={`w-2 h-2 rotate-45 border-b border-r ${r.border} bg-zinc-950 -mt-px`}
            />
          </div>
        </div>
      </div>
...
function LockedCard({ badge }) {
  ...
  return (
    ...
    <div
      className={`group relative rounded-2xl border ${r.border} bg-zinc-900/60 p-5 flex flex-col items-center gap-2 text-center h-[194px]`}
    >
      ...
      <p
        className={`text-xs font-semibold uppercase tracking-wide opacity-60 ${r.label}`}
      >
        {badge.rarity_level}
      </p>
      ...
      {/* Description - truncated, hover tooltip shows full text */}
      <div className="relative w-full">
        <p className="text-zinc-600 text-xs leading-snug line-clamp-2">
          {badge.description}
        </p>
        <div
          className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 z-50
      ...
      {isExclusive ? (
        <p className="text-[10px] text-zinc-600 mt-1 italic">
          Awarded exclusively - cannot be earned through activity
        </p>
      ) : (
        <div className="w-full mt-1">
          ...
          <p className="text-zinc-600 text-[10px] mt-1 text-right">
            {progress}% there
          </p>
        </div>
      )}
...
export default function Badges() {
  ...
  const featureMutation = useMutation({
    mutationFn: async ({ badge_code, featured }) => {
      ...
      if (featured) {
        const featuredCount = allBadges.filter(
          (b) => b.earned && b.is_featured,
        ).length;
        if (featuredCount >= 3)
          throw new Error("Max 3 badges can be featured. Unfeature one first.");
      }
      const res = await api.functions.invoke("getBadges", {
        action: "feature",
        badge_code,
    ...
    onSuccess: (_, { featured }) => {
      ...
      toast.success(
        featured ? "Badge featured on your profile!" : "Badge unfeatured.",
      );
    },
  ...
  const categories = [
    "all",
    ...new Set(allBadges.map((b) => b.category).filter(Boolean)),
  ];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-zinc-400">
            Earn badges through discipline, effort, and consistency
          </p>
        </div>
              ...
              <div
                ...
                style={{
                  width: allBadges.length
                    ? `${(earned.length / allBadges.length) * 100}%`
                    : "0%",
                }}
              />
            </div>
                ...
                {featured.length === 0 ? (
                  <div className="col-span-full text-center py-12">
                    ...
                    <p className="text-zinc-600 text-sm">
                      Go to Earned, hit "Feature" on any badge - it'll appear
