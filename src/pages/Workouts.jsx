    ...
    try {
      ...
      if (type === 'weekly') {
        const toDelete = workouts.filter(w =>
          (typeIds.weekly.has(w.id) || !!w.day_of_week) && !w.is_completed
        );
        ...
        const toDelete = workouts.filter(w =>
          (typeIds.single.has(w.id) || (!w.day_of_week && !typeIds.weekly.has(w.id))) && !w.is_completed
        );
        if (toDelete.length > 0) await Promise.all(toDelete.map(w => api.entities.Workout.delete(w.id)));
            ...
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5">
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-lg font-bold text-white mb-1">Generate AI Workout</h2>
