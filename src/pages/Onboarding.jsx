...
export default function Onboarding() {
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 4:
        return !!data.dietary_preference;
      case 5:
        return !!data.workout_preference;
      case 6:
        return regValid;
      default:
        return false;
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Promise.all([
        ...
        _api.functions.invoke("generateMealPlan", {
          calorieTarget: 2000,
  ...
  const handleSkip = async () => {
    try {
      ...
      if (subs?.[0]) {
        await _api.entities.Subscription.update(subs[0].id, {
          plan: "starter",
          status: "active",
        });
      }
            ...
            {data.date_of_birth &&
              (calculateAge(data.date_of_birth) < MIN_AGE ? (
                <p className="text-red-400 text-xs mt-2">
                  You must be at least {MIN_AGE} years old to use 7%.
                </p>
              ) : (
        ...
        <div className="space-y-6">
          <div>
            ...
            <Input
              ...
              onChange={(/** @type {any} */ e) =>
                update("height_cm", e.target.value)
              }
              min="120"
              max="220"
          ...
          options={[
            {
              ...
              label: "No Preference",
              icon: "🍽️",
              description: "All foods welcome",
            },
            {
              value: "vegetarian",
              label: "Vegetarian",
              icon: "🥗",
              description: "Plant-based + dairy",
            },
            {
              value: "vegan",
              label: "Vegan",
            ...
            },
            {
              ...
              label: "Gluten Free",
              icon: "🌾",
              description: "No wheat products",
            },
          ]}
          selected={data.dietary_preference}
          onSelect={(/** @type {string} */ v) =>
            update("dietary_preference", v)
          }
          columns={2}
        />
      ),
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                type="button"
                onClick={() => setReg((r) => ({ ...r, showPw: !r.showPw }))}
              ...
              <div className="mt-2">
                <div className="flex gap-1 mb-1">
                  {[1, 2, 3, 4].map((i) => (
                    <div
                      key={i}
                      className={`h-1 flex-1 rounded-full transition-all ${strength.score >= i ? strength.color : "bg-zinc-800"}`}
                    />
                  ))}
                </div>
                {strength.label && (
                  <p
                    className={`text-xs ${strength.label === "Weak" ? "text-red-400" : strength.label === "Fair" ? "text-amber-400" : strength.label === "Good" ? "text-yellow-400" : "text-green-400"}`}
                  >
    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="flex items-center justify-between mb-8">
          <button
            onClick={() =>
              step > 0 ? setStep((s) => s - 1) : navigate(createPageUrl("Home"))
            }
          ...
          <div>
            ...
            <div className="flex gap-1.5 mb-10">
              {Array.from({ length: totalSteps }).map((_, i) => (
                <div
                  key={i}
                  className={`h-1 flex-1 rounded-full transition-all duration-500 ${i <= step ? "bg-amber-400" : "bg-zinc-800"}`}
                />
              ))}
            </div>
            ...
            <p className="text-zinc-500 mb-8 text-sm leading-relaxed">
              Start your free trial to unlock all features - or skip and upgrade
              later.
            </p>
            ...
            <div className="rounded-2xl border-2 border-amber-500/40 bg-amber-500/5 p-6 mb-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-bold px-3 py-1 rounded-full bg-gradient-to-r from-green-400 to-emerald-500 text-black uppercase tracking-wide">
                  7 Day Free Trial
                </span>
              </div>
              ...
              <div className="mb-1">
                ...
                <span className="text-green-500/70 text-sm font-semibold ml-2">
                  for 7 days
                </span>
              </div>
              <p className="text-zinc-500 text-xs mb-5">
                then $12.99/month - cancel anytime before trial ends
              </p>
              ...
              >
                {checkingOut ? (
                  ...
                  <>
                    <CreditCard className="w-4 h-4 mr-2" /> Start 7-Day Free
                    Trial
                  </>
                )}
              </Button>
              <p className="text-center text-zinc-600 text-xs mt-2">
                No charge for 7 days • Cancel before trial ends
              </p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-zinc-700 text-xs mt-1">
                Features will be locked until you subscribe
              </p>
            </div>
          </div>
        )}
